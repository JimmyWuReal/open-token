...
/** Per-day token and cost totals, each split by provider, keyed `YYYY-MM-DD`. */
export type ProviderTotals = Map<
  string,
  { tokens: number; cost: number; byProvider: Map<string, number>; costByProvider: Map<string, number> }
>;

/** Sum tokens and cost (each split by provider) per calendar day (UTC), keyed `YYYY-MM-DD`. */
export function dailyProviderTotals(events: TokenEvent[]): ProviderTotals {
  const days: ProviderTotals = new Map();
  for (const event of events) {
    const day = event.timestamp.slice(0, 10);
...
export function lastDaysByProvider(totals: ProviderTotals, count: number): DayProviderTotal[] {
  const now = new Date();
  const todayUtc = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate());
